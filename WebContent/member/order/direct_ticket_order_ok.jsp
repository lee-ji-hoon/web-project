...
try {
	...
	String jsql2 = "select t_id, ct_qty_a, ct_qty_t, ct_qty_c from temp_cart_t where ct_no = ?";
	...
	{ // 주문수량(ordQty)을 저장하기 위해, 장바구니테이블(cart)로부터
		//  prdNo과 ctQty를 가져온다.
		String t_id = rs2.getString("t_id"); //  cart테이블로부터 상품번호 추출
		int ct_qty_a = rs2.getInt("ct_qty_a"); //  cart테이블로부터 주문수량 추출 
		int ct_qty_t = rs2.getInt("ct_qty_t");
		int ct_qty_c = rs2.getInt("ct_qty_c");

		String jsql3 = "INSERT INTO order_product (ord_no, p_id, ord_qty_a, ord_qty_t, ord_qty_c) VALUES (?,?,?,?,?)";
	...
	response.sendRedirect("direct_ticket_order_delete.jsp"); //  forward 액션태그를 사용해도 동일함
